...
# title
       ...
       # Payment Button
        btnPayment = Button(labelframeleft, text="Payment", command=self.open_payment_page, font=(
            "arial", 12, "bold"), bg="black", fg="#CD853F", width=10)
        btnPayment.grid(row=10, column=1, padx=1, sticky=W)
    ...
    def get_customer_data(self):
        ...
        print(f"Customer Mobile: {mobile_number}")
...
# All Data fetch
    ...
    # Create a new Toplevel window
        ...
        # Total Amount Label
        lbl_total_amount = Label(payment_window, text=f"Total Amount: {self.var_total.get()}", font=("arial", 14, "bold"))
        lbl_total_amount.pack(pady=10)

        # Payment Method Label and Dropdown
        lbl_payment_method = Label(payment_window, text="Select Payment Method:", font=("arial", 12))
        lbl_payment_method.pack(pady=10)

        payment_method = StringVar()
        combo_payment_method = ttk.Combobox(payment_window, textvariable=payment_method, font=("arial", 12), state="readonly", width=20)
        combo_payment_method["values"] = ("Cash", "Credit Card", "Debit Card", "UPI", "Net Banking")
        combo_payment_method.current(0)
        combo_payment_method.pack(pady=10)

        # Confirm Payment Button
        btn_confirm_payment = Button(payment_window, text="Confirm Payment", font=("arial", 12, "bold"), bg="green", fg="white",
                                    command=lambda: self.process_payment(payment_window, payment_method.get()))
        btn_confirm_payment.pack(pady=20)

        # Cancel Button
        btn_cancel_payment = Button(payment_window, text="Cancel", font=("arial", 12, "bold"), bg="red", fg="white",
                                    command=payment_window.destroy)
        btn_cancel_payment.pack(pady=10)
     
    def process_payment(self, payment_window, payment_method):
    # Logic to save payment details
        try:
            ...
            query = "INSERT INTO payments (Mobile, TotalAmount, PaymentMethod) VALUES (%s, %s, %s)"
            my_cursor.execute(query, (self.var_mobile.get(), self.var_total.get(), payment_method))
            conn.commit()
            conn.close()

            # Show success message
            messagebox.showinfo("Success", f"Payment successful using {payment_method}!", parent=payment_window)

            # Close payment page
            payment_window.destroy()
        except Exception as es:
            messagebox.showerror("Error", f"Error during payment: {str(es)}", parent=payment_window)
